    ...
    abstract class VorbisFloor
    {
        ...
        class Floor0 : VorbisFloor
        {
            ...
            internal override void Apply(PacketData packetData, float[] residue)
            {
                ...
                {
                    // this is pretty well stolen directly from libvorbis... BSD license
                    int[] barkMap = _barkMaps[data.BlockSize];
                    float[] wMap = _wMap[data.BlockSize];
        ...
        class Floor1 : VorbisFloor
        {
            ...
            readonly static int[] _rangeLookup = { 256, 128, 86, 64 };
            readonly static int[] _yBitsLookup = { 8, 7, 7, 6 };
            ...
            bool[] UnwrapPosts(PacketData1 data)
            {
                ...
                for (int i = 2; i < data.PostCount; i++)
                {
                    ...
                    int predicted = RenderPoint(_xList[lowOfs], _finalY[lowOfs], _xList[highOfs], _finalY[highOfs], _xList[i]);
                    ...
                    if (val != 0)
                    {
                        ...
                        {
                            if ((val % 2) == 1)
                                _finalY[i] = predicted - ((val + 1) / 2); // odd
                            else
                                _finalY[i] = predicted + (val / 2); // even
                        }
                    }
                    else
                    {
